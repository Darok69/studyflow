...
export const de: Messages = {
  ...
  navReader: 'Lehrbuch',
  readerTitle: 'Lehrbuch',
  readerLead: 'Die Folie, darunter die Erklärung, die Begriffe und am Ende die Fragen. Die Stelle wird gemerkt.',
  readerLectures: (n: number) => `${n} ${n === 1 ? 'Vorlesung' : 'Vorlesungen'}`,
  readerKind_lecture: 'Vorlesung',
  readerKind_exercise: 'Übung',
  readerKind_reading: 'Lektüre',
  readerKind_id: '',
  readerLoose: 'Zur Prüfung insgesamt',
  readerSlideCount: (n: number) => `${n} ${n === 1 ? 'Folie' : 'Folien'}`,
  readerCardCount: (n: number) => `${n} ${n === 1 ? 'Karte' : 'Karten'}`,
  readerContinue: (n: number) => `Weiter bei Folie ${n}`,
  readerTerms: (n: number) => `Begriffe (${n})`,
  readerQuestions: (n: number) => `Fragen (${n})`,
  readerCoreOnly: 'nur Kern',
  readerEmpty: 'Auf dem Server liegen noch keine Unterlagen.',
