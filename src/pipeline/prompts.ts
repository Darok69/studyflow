...
export function outlinePrompt({ subjectName, discipline, blocks }: OutlinePromptInput): string {
  return [
    ...
    '',
    'Níže jsou ZAČÁTKY bloků podkladu (delší bloky jsou zkrácené — na osnovu to stačí).',
    'Navrhni osnovu témat ke studiu:',
...
export function cardsPrompt({
  ...
  return [
    ...
    'Ke každé kartě uveď blockId bloku, ze kterého vychází, a do pole evidence',
    'ZKOPÍRUJ doslovný úryvek z podkladu (jednu větu), na kterém odpověď stojí.',
    'Úryvek musí být přesně z textu níže — kontroluje se strojově a karta bez opory se odloží.',
    '',
    '--- BLOKY TÉMATU ---',
    renderBlocks(blocks),
  ].join('\n')
